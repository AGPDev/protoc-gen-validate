...
public class {{ classNameFile . }}Validator {
	public static io.AGPDev.pgv.ValidatorImpl validatorFor(Class clazz) {
		{{ range .AllMessages }}
		{{ if not (ignored .) -}}
...
*/
@SuppressWarnings("all")
public class {{ classNameMessage .}}Validator implements io.AGPDev.pgv.ValidatorImpl<{{ qualifiedName . }}>{
	public static io.AGPDev.pgv.ValidatorImpl validatorFor(Class clazz) {
		if (clazz.equals({{ qualifiedName . }}.class)) return new {{ simpleName .}}Validator();
		{{ range .AllMessages }}
		{{ if not (ignored .) -}}
